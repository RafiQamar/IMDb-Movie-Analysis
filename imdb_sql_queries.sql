...
FROM imdb_movies m1
WHERE ratings = (SELECT max(ratings) from imdb_movies m2 WHERE m1.main_genre = m2.main_genre);
...
FROM imdb_movies m1
WHERE `duration(min)` = (SELECT max(`duration(min)`) from imdb_movies m2 WHERE m1.main_genre = m2.main_genre);
...
FROM imdb_movies m1
WHERE ratings = (SELECT MAX(ratings) FROM imdb_movies m2 WHERE m1.rated = m2.rated);